...
```{r}
#| label: Reference - creating summary tables
#| eval: true
#| echo: false
#| message: false
#| warning: false
...
#-------this chunk is just for reference to see how the summary tables were created

##----Sampling effort summary----
# Set to FALSE to count stationary detector sites only (drops mobile transects)
include_transects <- TRUE

raw <- read_excel("Data/Raw/AllYearActivitybyNight_2016_2025_BC_withALLmetadata.xlsx", sheet = "Occupancy_NEAR")

effort_nights <- raw %>%
  select(GRTS_Cell_ID, Quadrant, Night) %>%
  mutate(Night = as.Date(Night),
         Year  = year(Night)) %>%
  distinct(GRTS_Cell_ID, Quadrant, Night, .keep_all = TRUE) %>%
  # Drop nights with bad detector clock dates (1963, 2000, 2001, 2015)
  filter(between(Year, 2016, 2025)) %>%
  { if (include_transects) . else filter(., Quadrant != "Transects") }

samplingE <- effort_nights %>%
  group_by(Year) %>%
  summarise(
    `Number of Grids`       = n_distinct(GRTS_Cell_ID),
    `Number of Site Nights` = n(),   # one row = one site (cell + quadrant) x night
    .groups = "drop"
  ) %>%
  arrange(Year)

write.csv(samplingE, "Data/Analyzed/SummarySamplingEffort.csv")

##---Summary for map----
#only using full species ID not couplets
sp_cols <- c("ANPA", "COTO", "EPFU", "EUMA", "LABO", "LACI", "LANO", "MYCA", "MYCI", "MYEV", "MYLU", "MYTH", "MYVO", "MYSE", "MYYU", "PAHE", "TABR")
site_list <-raw %>%
  # Clean nightly data: one row per site x night
  mutate(year = lubridate::year(Night)) %>%
  distinct(GRTS_Cell_ID, Quadrant, Night, .keep_all = TRUE) %>%   # drop duplicates
  filter(between(year, 2016, 2025),                     # drop bad detector dates
         Quadrant != "Transects",                       # transects drawn as lines
         !is.na(Latitude), !is.na(Longitude)) %>%
  transmute(grts_id  = as.character(GRTS_Cell_ID),
            quadrant = as.character(Quadrant),
            site_name = paste(grts_id, quadrant),
            Lat = Latitude, Long = Longitude, year,
            across(all_of(sp_cols))) %>%
  pivot_longer(all_of(sp_cols), names_to = "species", values_to = "recordings") %>%
  # recordings per species per site per year
  group_by(site_name, grts_id, quadrant, Lat, Long, year, species) %>%
  summarise(n = sum(recordings, na.rm = TRUE), .groups = "drop") %>%
  # one row per site
  group_by(site_name, grts_id, quadrant, Lat, Long) %>%
  summarise(
    Years_Sampled = paste(sort(unique(year)), collapse = ", "),
    N_Years       = n_distinct(year),
    years         = list(sort(unique(year))),
    total         = sum(n),
    sp_data       = list(tibble(species, n, year) %>% filter(n > 0)),
    .groups = "drop"
  )

saveRDS(site_list, "Data/Analyzed/MapDataSummary.rds")

#for a readable version save as csv.
site_list %>%
  select(-years) %>%
  tidyr::unnest(sp_data) %>%
  write.csv("Data/Analyzed/MapDataSummary.csv", row.names = FALSE)
```
...
#upload summary data for map
site_list <- readRDS("Data/Analyzed/MapDataSummary.rds")
...
# ── 4. SVG pie helper ──────────────────────────────────────────────────────────
make_pie_svg <- function(values, colors, size = 96) {
  total <- sum(values)
...
make_sp_table <- function(sp_df, pie_colors) {
  sp_df %>%
    ...
    purrr::pmap_chr(function(species, n, pct) sprintf(
      '<tr><td style="padding-right:8px"><span style="display:inline-block;width:10px;height:10px;border-radius:50%%;background:%s;margin-right:4px"></span>%s</td><td align="right" style="padding-right:6px">%s</td><td align="right">%s%%</td></tr>',
      ifelse(species %in% names(pie_colors), pie_colors[species], "#AAAAAA"),
      species, format(n, big.mark = ","), pct
    )) %>%
    paste(collapse = "")
}

# ── 6. (collapse_pie kept for reference; no longer used in popups) ──────────────
...
for (i in seq_len(nrow(site_list))) {
  ...
  # Skip sites with no detections at all
  if (is.null(sp) || nrow(sp) == 0 || sum(sp$n, na.rm = TRUE) == 0) next
  
  # -- Build one pie + species table per year (most recent first) --------------
  year_blocks <- purrr::map_chr(sort(years, decreasing = TRUE), function(yr) {
    sp_yr <- sp %>%
      filter(year == yr) %>%
      group_by(species) %>%
      summarise(n = sum(n), .groups = "drop") %>%
    ...
    if (nrow(sp_yr) == 0 || sum(sp_yr$n) == 0) return("")
    
    yr_total <- sum(sp_yr$n)
    
    # Colours: known palette + fallback for anything unmapped
    yr_species <- sp_yr$species
    unknown    <- setdiff(yr_species, names(sp_colors))
    fallback   <- setNames(
      colorRampPalette(brewer.pal(8, "Dark2"))(max(length(unknown), 1)),
      unknown)
    yr_colors  <- c(sp_colors, fallback)
    
    # Accumulate for the map-level legend (section 9)
    all_pie_colors <<- c(all_pie_colors, yr_colors[yr_species])
    
    uri         <- make_pie_svg(sp_yr$n, yr_colors[yr_species], size = 96)
    legend_rows <- make_sp_table(sp_yr, yr_colors)
    
    sprintf(
      '<div style="margin-bottom:14px;">
         <div style="font-weight:600;font-size:13px;margin-bottom:6px;color:#333;">%s</div>
         <div style="display:flex;align-items:flex-start;gap:12px;">
           <img src="%s" width="96" height="96" style="flex-shrink:0;"/>
           <table style="font-size:11px;border-collapse:collapse;">
             <tr><th align="left" style="padding-right:8px">Species</th><th align="right" style="padding-right:6px">n</th><th align="right">%%</th></tr>
             %s
           </table>
         </div>
         <div style="font-weight:600;font-size:12px;margin-top:4px;color:#333;">Total bats: %s</div>
       </div>',
      yr, uri, legend_rows, format(yr_total, big.mark = ",")
    )
  }) %>% paste(collapse = "")
  
  # -- Popup: stacked yearly pies ----------------------------------------------
  popup <- sprintf(
    '<div style="font-family:sans-serif;min-width:270px;max-width:340px;max-height:420px;overflow-y:auto;">
       <b style="font-size:14px;">%s</b>
       <hr style="margin:6px 0"/>
       %s
     </div>',
    row$site_name, year_blocks
  )
...
# ── 9. Load spatial data ──────────────────────────────────────────────────────────
transects_sf <- st_read("Data/Mapping/transects_merged.shp", quiet = TRUE)
transects_sf <- st_transform(transects_sf, crs = 4326)

regions <- st_read("Data/Mapping/BC-SEAK Hub Regions Mar 2025.shp", quiet = TRUE)
...
m_full <- m %>%
  # Custom panes: regions/transects sit BELOW circle markers (overlayPane = 400)
  addMapPane("regionsPane",   zIndex = 240) %>%
  addMapPane("transectsPane", zIndex = 260) %>%
  addPolygons(
    data        = regions,
    ...
    options     = pathOptions(pane = "regionsPane")
  ) %>%
  addPolylines(
    data             = transects_sf,
    color            = "darkgreen",
    ...
    group   = "Transects",
    options = pathOptions(pane = "transectsPane")
  ) %>%
  addLayersControl(
    overlayGroups = c("Analysis Regions", "Transects"),
